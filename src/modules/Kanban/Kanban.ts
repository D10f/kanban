...
export default class Kanban {
  ...
  constructor({ selector, boards }: KanbanParams) {
    ...
    this.wrapper = this.section.querySelector(
      `${selector}__wrapper`,
    ) as HTMLElement;
    this.boards = boards || [];
  ...
  private dragEvents() {
    this.wrapper.addEventListener("dragstart", (event) => {
      ...
      // this.draggedItemRect = draggedItem.getBoundingClientRect();
    });
    ...
    this.wrapper.querySelectorAll("article").forEach((board) => {
      ...
      board.addEventListener("dragover", (event) => {
        ...
        const items = board.getElementsByTagName("li");
        let target: Element | null = null;

        for (const item of items) {
          const itemRect = item.getBoundingClientRect();
          if (itemRect.top < mouseOffsetY) {
            target = item;
          }
        }
  ...
  private createTaskModal(selector: string) {
    ...
    newTaskModal.onSubmit = (dialog: HTMLDialogElement) => {
      const descriptionEl = dialog.querySelector(
        "[name=description]",
      ) as HTMLInputElement;
  ...
  private render() {
    ...
    this.boards.forEach((board) => {
      fragment.appendChild(board.boardEl);
    });
